...
```{r}
# Creating linear regression model by using the county dataset and piping a linear regression formula,
# in which we set the dependent variable (before tilda) to poverty and the independent variable (after tilda) 
# to less_than_hs.

poverty_model <- county %>%
  lm(poverty ~ less_than_hs, data= .)

# Creating a regression table by referencing our linear regression model from above and using the tidy function
# in the broom package, through which we can get a range of statistical output by setting the conf.int variable equal to TRUE.

poverty_model %>%
  tidy(conf.int = TRUE)
```

The intercept is 6.64, which is intercept of poverty with less than high school degree at 0. The slope of the regression is 0.63, the more people that have a less than high school degree, the poverty rate is 0.63.
...
# Created a bootstrap sample of the county data with 3 replicates (instead of 1000 because computers may crash). 
# Grouped by replicate and nested to create individual tibbles for each replicate.
...
# Created a column to find the number of distinct rows in each replicate by using the map_dbl
# function on the "data" column from our bootstrap sample above and using n_distinct on that data
